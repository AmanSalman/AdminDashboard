...
function UserOrders() {
  ...
  const userOrders = async () => {
    try {
      setIsLoading(true);
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL2}/order/admin/userorders/${id}`, {
        headers: {
          Authorization: `AmanGRAD__${token}`
        }
      });
    ...
    <main className="container my-4">
      ...
      <div className="border p-4 bg-light">
        <Link to="/users" className="btn btn-outline-primary mb-3">
          <TbArrowBigLeftLineFilled className="me-2" /> Back to Users
        </Link>
